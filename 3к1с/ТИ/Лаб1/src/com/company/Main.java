...
public class Main {
    ...
    protected static int countEntropy;

    public static void main(String[] args) {

        String text = "Привет, как дела?";

        countEntropy = text.length();
        ...
        for(Map.Entry<Character, String> entry : codes.entrySet()) {
            ...
            length += value.length();
            count++;
            kraft += Math.pow(encoded.length(), -(value.length()));
        }
        double mediumLength = length / count;
        System.out.println("Средняя длинна кодового слова : " + mediumLength);
        if(kraft <= 1)
            System.out.println("Условие Крафта-Макмиллана выполняется");
    ...
    private static void fileCompressTest() {
        try {
            ...
            String content = new String(Files.readAllBytes(Paths.get("./src/com/company/Первоначальный_текст.txt")));
            ...
            System.out.println("Энтропия для файла : " + entropy());
            ...
            File file = new File(".\\src\\com\\company\\compressed.huf");
            ...
            Files.write(Paths.get("./src/com/company/Декодированный_текст.txt"), decoded.getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static double entropy() {
        ...
        for (Integer freq: frequencies.values()) {
            double probability = (double)freq / (double)countEntropy;
